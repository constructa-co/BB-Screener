    ...
    async def _analyze_funding_sentiment(self) -> Dict:
        ...
        try:
            ...
            for exchange_name, exchange in self.exchanges.items():
                self.logger.info(f"📡 Attempting funding rate from {exchange_name}...")
                try:
                    ...
                    if exchange_name == 'binance':
                        ...
                        self.logger.info(f"✅ Binance funding rate SUCCESS: {current_rate}")
                        ...
                        self.logger.info(f"✅ Bybit funding rate SUCCESS: {current_rate}")
                        ...
                        self.logger.info(f"✅ OKX funding rate SUCCESS: {current_rate}")
                    
                    funding_data[exchange_name] = current_rate
                    
                except Exception as e:
                    self.logger.error(f"❌ {exchange_name} funding rate FAILED: {e}")
                    continue
            
            # Calculate aggregate funding sentiment
            self.logger.info(f"📊 Collected funding data: {funding_data}")
            if funding_data:
                avg_funding = np.mean(list(funding_data.values()))
                self.logger.info(f"📈 Average funding rate: {avg_funding}")

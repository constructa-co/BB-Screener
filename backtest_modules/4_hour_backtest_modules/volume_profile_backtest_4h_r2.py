...
from vp_trade_output_module import VolumeProfileTradeReporter
...
class VolumeProfileBacktest:
    ...
    def check_volume_confirmation(self, df, idx, strategy):
        ...
        FIX: Volume should be HIGH for breakouts, not rejected for being high
        """
        if idx < 20:
            return False, 0.0
        ...
        thresholds = {
            'mean_reversion': 1.5,  # Want moderate volume
            'breakout': 1.3,        # Want high volume for breakouts
            'poc_magnet': 1.5,
            'virgin_poc': 1.5,
            'composite': 1.5
        }
        
        min_threshold = thresholds.get(strategy, 1.5)
        
        # FIXED LOGIC: Different strategies need different volume conditions
        if strategy == 'breakout':
            # Breakouts NEED high volume to confirm
            if volume_ratio >= min_threshold:
                if self.debug and volume_ratio > 2.0:
            ...
            # Mean reversion works best with moderate volume (not too high, not too low)
            if 0.8 <= volume_ratio <= 2.5:  # Allow range for mean reversion
                return True, volume_ratio
            else:
                if self.debug and volume_ratio > 2.5:
                    print(f"⚠️ Volume too high for mean_reversion: {volume_ratio:.2f}x")
                return False, volume_ratio
                
        else:
            # Other strategies: POC Magnet, Virgin POC, Composite
            # These work with normal to slightly elevated volume
            if 0.7 <= volume_ratio <= 3.0:  # Wider range for these strategies
                return True, volume_ratio
            else:
                                return False, volume_ratio
    ...
    def mean_reversion_strategy(self, df: pd.DataFrame, idx: int, vp: Dict) -> Optional[Dict]:
        ...
        # Calculate entry score with LOWER threshold
        score = self.calculate_entry_score(df, idx, vp, 'mean_reversion')
        
        # REDUCED: Accept 65+ scores instead of 70+
        if score < 65:
            return None
    ...
    def breakout_strategy(self, df: pd.DataFrame, idx: int, vp: Dict) -> Optional[Dict]:
        """FIXED breakout trading beyond Value Area"""
        if not vp or idx < 50:
            return None
        ...
        breakout_up = (prev['close'] < vp['vah'] and 
                       current['close'] > vp['vah'] and
                       current['close'] > vp['vah'] * 1.005)  # Must close 0.5% above
        
        breakout_down = (prev['close'] > vp['val'] and 
                         current['close'] < vp['val'] and
                         current['close'] < vp['val'] * 0.995)  # Must close 0.5% below
        ...
        # IMPROVED breakout targets
        if breakout_up:
            direction = 'long'
            entry = current['close']
            stop = vp['vah'] - atr * 1.0  # Tighter stop
            
            # Find next resistance or use ATR-based target
            potential_targets = [h for h in vp.get('hvn', []) if h > entry * 1.01]
            if potential_targets:
                target = min(potential_targets)
            else:
                # Use 2.5x ATR move for target
            ...
            # Find next support or use ATR-based target
            potential_targets = [h for h in vp.get('hvn', []) if h < entry * 0.99]
            if potential_targets:

...
"""
Volume Profile Backtest - R6 RISK MANAGEMENT & PERFORMANCE FIXES
===============================================================

CRITICAL FIXES APPLIED:
1. IMPROVED P&L CALCULATION WITH PROPER EXIT LOGIC
   - calculate_trade_pnl now uses actual stop/target levels
   - Respects R/R ratios from research (1.5-2:1 for mean reversion, 3:1 for breakout)
   - Uses high/low prices for accurate exit simulation
   - Returns comprehensive trade info with win/loss status

2. ADDED POSITION SIZING & RISK MANAGEMENT
   - calculate_position_size: 1% risk per trade
   - execute_trades_with_risk_management: Proper risk controls
   - Max 3 concurrent trades to prevent overexposure
   - Account balance tracking with drawdown calculation

3. COMPREHENSIVE PERFORMANCE METRICS
   - calculate_performance_metrics: Proper drawdown tracking
   - Profit factor, expectancy, Sharpe ratio calculation
   - Account balance progression with peak tracking
   - Risk-adjusted return metrics

EXPECTED IMPROVEMENTS:
- Reduce max drawdown from 88% to under 20%
- Improve profit factor from 1.03 to 1.5+
- Better risk-adjusted returns with Sharpe ratio > 0.5
...
RISK MANAGEMENT:
- 1% risk per trade (prevents catastrophic losses)
- Position sizing based on stop loss distance
- Max 10x leverage to prevent overleveraging
- Concurrent trade limits to manage exposure
"""
...
class VolumeProfileBacktest:
    ...
    def calculate_trade_pnl(self, trade, df, entry_idx):
        """
        Calculate P&L for a trade using proper exit logic
        Research-aligned: Use actual R/R ratios and respect stops/targets
        """
        ...
        for i in range(entry_idx + 1, min(entry_idx + 100, len(df))):  # Max 100 bars
            high = df.iloc[i]['high']
            low = df.iloc[i]['low']
        ...
        # If neither stop nor target hit, exit at current price (timeout)
    ...
    def execute_trades_with_risk_management(self, df, signals, vp_data, commission=0.001):
        ...
        for signal in signals:
            ...
            # Strategy-specific R/R requirements
            min_rr = {
                'mean_reversion': 1.0,
                'breakout': 2.0,
                'poc_magnet': 0.8,
            ...
            if rr_ratio < min_rr.get(strategy, 1.0):
                continue
    ...
    def calculate_performance_metrics(self, trades, initial_balance=10000):
        """
        Calculate comprehensive performance metrics with proper drawdown tracking
        """
        if not trades:
            return {}
        ...
        for trade in sorted_trades:
            # Calculate trade P&L in dollars
            pnl_dollars = (trade['pnl_pct'] / 100) * trade['entry_price'] * trade.get('position_size', 1)
            balance += pnl_dollars

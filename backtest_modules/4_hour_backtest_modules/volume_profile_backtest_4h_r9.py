...
"""
Volume Profile Backtest - R9 SURGICAL OPTIMIZATION
==================================================

R9 OPTIMIZATIONS BASED ON R8 ANALYSIS:

1. TIGHTER ENTRY ZONES (1.0x ATR)
   - Reduced from 1.5x to 1.0x ATR for better R/R
   - Zone quality filter: only enter in first 50% of zone
   - Should reduce trades to 200-250 (higher quality)

2. WIDER STOP LOSSES (3.0x ATR)
   - Increased from 2.5x to 3.0x ATR minimum
   - Place stops 1 ATR beyond nearest HVN (was 2.5x)
   - Should fix R/R rejection issues

3. DYNAMIC TARGET SELECTION
   - Mean Reversion: POC first, then VAH/VAL with 2:1 minimum
   - Breakout: 4:1 minimum R/R
   - POC strategies: dynamic based on structure

4. ENTRY QUALITY FILTER
   - Only take entries with zone_quality >= 0.5
   - Pre-filter R/R before creating signals
   - Minimum 1.5:1 R/R for all strategies

5. REDUCED PRICE TOLERANCE
   - BTC: 1% (down from 1.5%)
   - ETH: 1.5% (down from 2%)
   - Altcoins: 2% (down from 3%)

EXPECTED R9 RESULTS:
- Trade Count: 200-250 (down from 336)
- Overall Win Rate: 45-50% (up from 36.3%)
- Mean Reversion: 55-65% win rate
- Max Drawdown: Under 40% (down from 59.94%)
- Profit Factor: > 1.2 (up from 0.80)

KEY FIXES:
- Tighter zones for better entry positioning
- Wider stops for better R/R ratios
- Dynamic targets based on market structure
- Quality filtering for higher win rates
- Pre-filter R/R to avoid rejections
"""
...
class VolumeProfileBacktest:
    ...
    def __init__(self, data_path: str, output_path: str):
        ...
        # R9 PARAMETERS - Tighter zones, wider stops for better R/R
        self.config = {
            # Profile calculation
            ...
            # TIGHTENED: Volume and node thresholds
            'min_volume_node': 0.02,        # BACK to 0.02 (2% for quality)
            'max_distance_pct': 0.05,       # BACK to 0.05 (5% for precision)
            
            # TIGHTENED: Entry zones for better R/R
            'entry_zone_atr': 1.0,          # REDUCED from 2.0 (tighter zones)
            'stop_loss_atr': 3.0,           # INCREASED from 2.0 (wider stops)
            'min_rr_ratio': 1.5,            # KEEP 1.5 (minimum good R/R)
    ...
    def calculate_stop_loss(self, df, vp_data, entry_price, signal_type, idx):
        """
        Wider stops for better R/R ratios
        R9: 3.0x ATR minimum, 1 ATR beyond nearest HVN
        """
        atr = df.iloc[idx]['atr']
        ...
        if signal_type == 'long':
            ...
            if hvns:
                ...
                if hvns_below:
                    ...
                    # Place stop 1 ATR below HVN (was 2.5x)
                    stop_loss = nearest_hvn - atr
                else:
                    # No HVN found, use 3x ATR
                    stop_loss = entry_price - (atr * 3.0)
            else:
                stop_loss = entry_price - (atr * 3.0)
            
            # Ensure minimum 3% distance for majors
            min_distance = entry_price * 0.03
    ...
    def calculate_target(self, df, vp_data, entry_price, signal_type, strategy, idx):
        """
        Dynamic targets based on strategy and market structure
        """
        atr = df.iloc[idx]['atr']
        ...
        if strategy == 'mean_reversion':
            ...
            if signal_type == 'long':
                target = entry_price + (atr * 4)  # 4:1 minimum
            else:
                target = entry_price - (atr * 4)
    ...
    def check_entry_zone(self, price, level, atr, zone_type='standard'):
        ...
        # R9: Tighter zone multipliers
        if zone_type == 'tight':
            zone_multiplier = 0.5   # Very tight
        elif zone_type == 'standard':
            zone_multiplier = 1.0   # Tighter than R8's 1.5x
        elif zone_type == 'wide':
            zone_multiplier = 1.5   # For volatile conditions
        else:
            zone_multiplier = 1.0
        ...
        # R9: Reduced price tolerance
        if price > 10000:  # BTC range
            price_tolerance = price * 0.01   # 1% (was 1.5%)
        elif price > 1000:  # ETH range
            price_tolerance = price * 0.015  # 1.5% (was 2%)
        else:  # Altcoins
            price_tolerance = price * 0.02   # 2% (was 3%)
        
        # Total zone size
        total_zone = atr_zone + price_tolerance
        distance = abs(price - level)
        
        # R9: Zone quality (how close to the level)
            ...
            if volume_confirmed:  # Will be True if volume >= 1.5x
                ...
                if score >= 80:  # Only take high-quality breakouts
                    ...
                    if rr_ratio >= 1.5:  # Only good R/R
                        ...
                        print(f"⛔ Breakout rejected - R/R {rr_ratio:.2f} < 1.5")
    ...
    def detect_mean_reversion_signals(self, df, idx, vp):
        ...
        if in_zone and zone_quality > 0.5:  # Only good entries
            score = self.calculate_entry_score(df, idx, vp, 'mean_reversion')
            ...
            if score >= 70:
                ...
                if rr_ratio >= 1.5:  # Only good R/R
                    ...
                    print(f"⛔ Trade rejected - R/R {rr_ratio:.2f} < 1.5")
            ...
            if volume_confirmed:
                ...
                if score >= 80:
                    ...
                    if rr_ratio >= 1.5:  # Only good R/R
                        ...
                        print(f"⛔ POC Magnet rejected - R/R {rr_ratio:.2f} < 1.5")
                ...
                if volume_confirmed:
                    ...
                    if score >= 75:  # Lower threshold for virgin POC
                        ...
                        if rr_ratio >= 1.5:  # Only good R/R
                            ...
                            print(f"⛔ Virgin POC rejected - R/R {rr_ratio:.2f} < 1.5")

...
class VolumeProfileBacktest:
    ...
    def __init__(self, data_path: str, output_path: str):
        ...
        self.config = {
            ...
            'entry_zone_atr': 1.5,     # 1.5x ATR for zone entries
            'stop_loss_atr': 2.5,      # 2.5x ATR for stop loss
            'min_rr_ratio': 2.0,       # Minimum 2:1 R/R
            'volume_surge': 1.2,       # 120% volume confirmation
            'score_threshold': 80,     # Minimum score for entry
    ...
    def poc_magnet_strategy(self, df: pd.DataFrame, idx: int, vp: Dict) -> Optional[Dict]:
        ...
        # Check if we're in a balanced market (not trending strongly)
        if current['adx'] > 30:
            return None
    ...
    def virgin_poc_strategy(self, df: pd.DataFrame, idx: int, vp: Dict) -> Optional[Dict]:
        ...
        # Find virgin POCs
        virgin_pocs = self.find_virgin_pocs(df.iloc[:idx], 5)

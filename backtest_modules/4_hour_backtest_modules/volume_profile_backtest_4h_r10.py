...
"""
Volume Profile Backtest - R10 BALANCED OPTIMIZATION
===================================================

R10 FIXES BASED ON R9 ANALYSIS:

1. BALANCED ENTRY ZONES (1.2x ATR)
   - Increased from 1.0x to 1.2x ATR for achievable R/R
   - Zone quality filter: only enter in first 60% of zone
   - Should find 250-300 trades (good balance)

2. REASONABLE STOP LOSSES (2.5x ATR)
   - Reduced from 3.0x to 2.5x ATR (back to R8 level)
   - Place stops 1.5x ATR beyond nearest HVN
   - Should allow achievable R/R ratios

3. REALISTIC TARGET SELECTION
   - Mean Reversion: POC with 1.8:1 minimum
   - Breakout: 2.5:1 minimum R/R
   - POC strategies: POC with 1.5:1 minimum

4. RELAXED R/R REQUIREMENTS
   - Minimum 1.2:1 R/R for all strategies (down from 1.5)
   - Pre-filter R/R but allow reasonable trades
   - Focus on quality over extreme ratios

5. BALANCED PRICE TOLERANCE
   - BTC: 1.2% (up from 1%)
   - ETH: 1.8% (up from 1.5%)
   - Altcoins: 2.5% (up from 2%)

EXPECTED R10 RESULTS:
- Trade Count: 250-300 (up from 25)
- Overall Win Rate: 40-45% (up from 12%)
- Mean Reversion: 50-60% win rate
- Max Drawdown: Under 50% (manageable)
- Profit Factor: > 1.0 (profitable)

KEY FIXES:
- Balanced zones for achievable R/R
- Reasonable stops for better ratios
- Realistic targets for more trades
...
class VolumeProfileBacktest:
    ...
    def __init__(self, data_path: str, output_path: str):
        ...
        self.config = {
            ...
            # BALANCED: Volume and node thresholds
            'min_volume_node': 0.02,        # Keep 0.02 (2% for quality)
            'max_distance_pct': 0.05,       # Keep 0.05 (5% for precision)
            
            # BALANCED: Entry zones for achievable R/R
            'entry_zone_atr': 1.2,          # INCREASED from 1.0 (balanced zones)
            'stop_loss_atr': 2.5,           # REDUCED from 3.0 (reasonable stops)
            'min_rr_ratio': 1.2,            # REDUCED from 1.5 (relaxed R/R)
    ...
    def calculate_stop_loss(self, df, vp_data, entry_price, signal_type, idx):
        """
        Reasonable stops for achievable R/R ratios
        R10: 2.5x ATR minimum, 1.5x ATR beyond nearest HVN
        """
        atr = df.iloc[idx]['atr']
        
        # Find HVNs for better stop placement
        if vp_data and 'volume_nodes' in vp_data:
            ...
            hvns = [price for price, vol in volume_nodes.items() if vol > avg_volume * 1.2]
        else:
            hvns = []
        
        if signal_type == 'long':
            # For longs, find support below
            if hvns:
                hvns_below = [hvn for hvn in hvns if hvn < entry_price - atr]
                if hvns_below:
                    nearest_hvn = max(hvns_below)
                    # Place stop 1.5x ATR below HVN (was 1x)
                    stop_loss = nearest_hvn - (atr * 1.5)
                else:
                    # No HVN found, use 2.5x ATR
                    stop_loss = entry_price - (atr * 2.5)
            else:
                stop_loss = entry_price - (atr * 2.5)
            
            # Ensure minimum 2.5% distance for majors
            min_distance = entry_price * 0.025
            stop_loss = min(stop_loss, entry_price - min_distance)
            
        else:  # short
            if hvns:
                hvns_above = [hvn for hvn in hvns if hvn > entry_price + atr]
                if hvns_above:
    ...
    def calculate_target(self, df, vp_data, entry_price, signal_type, strategy, idx):
        """
        Realistic targets based on strategy and market structure
        """
        atr = df.iloc[idx]['atr']
        ...
        if strategy == 'mean_reversion':
            if signal_type == 'long':
                # From VAL, target POC first, then VAH
                primary_target = poc if poc > entry_price else vah
                # Ensure minimum 1.8:1 R/R
                min_target = entry_price + (atr * 1.8)
                target = max(primary_target, min_target)
            else:  # short from VAH
                primary_target = poc if poc < entry_price else val
                min_target = entry_price - (atr * 1.8)
                target = min(primary_target, min_target)
                
        elif strategy == 'breakout':
            # Breakouts need larger targets
            if signal_type == 'long':
                target = entry_price + (atr * 2.5)  # 2.5:1 minimum
            else:
                target = entry_price - (atr * 2.5)
                
        else:  # POC strategies
            if signal_type == 'long':
                target = poc if poc > entry_price else entry_price + (atr * 1.5)
            else:
                target = poc if poc < entry_price else entry_price - (atr * 1.5)
        
        return target

    def filter_high_quality_entries(self, signals, zone_quality_threshold=0.5):
        """
        Only take entries in the best part of the zone
        """
        filtered_signals = []
        
        for idx, signal in signals:
            # Check zone quality (how close to the level)
            zone_quality = signal.get('zone_quality', 0)
            
            if zone_quality >= zone_quality_threshold:
                filtered_signals.append((idx, signal))
            elif self.debug:
                print(f"⛔ Entry rejected - zone quality {zone_quality:.2f} < {zone_quality_threshold}")
        
        return filtered_signals

    def check_entry_zone(self, price, level, atr, zone_type='standard'):
        """
        Tighter zones for better entry positioning
        R9: 1.0x ATR zones with zone quality filter
        """
        if atr <= 0:
            return False, 0
            
        # R10: Balanced zone multipliers
        if zone_type == 'tight':
            zone_multiplier = 0.8   # Balanced tight
        elif zone_type == 'standard':
            zone_multiplier = 1.2   # Balanced (was 1.0)
        elif zone_type == 'wide':
            zone_multiplier = 1.8   # For volatile conditions
        else:
            zone_multiplier = 1.2
        
        # Base ATR zone
        atr_zone = atr * zone_multiplier
        
        # R10: Balanced price tolerance
        if price > 10000:  # BTC range
            price_tolerance = price * 0.012  # 1.2% (was 1%)
        elif price > 1000:  # ETH range
            price_tolerance = price * 0.018  # 1.8% (was 1.5%)
        else:  # Altcoins
            price_tolerance = price * 0.025  # 2.5% (was 2%)
        
        # Total zone size
        total_zone = atr_zone + price_tolerance
        distance = abs(price - level)
        
        # R10: Zone quality (how close to the level) - relaxed threshold
        in_zone = distance <= total_zone
        zone_quality = 1.0 - (distance / total_zone) if in_zone else 0
        
        if self.debug and in_zone:
            print(f"✅ Price in zone: distance={distance:.2f}, zone={total_zone:.2f}, quality={zone_quality:.2f}")
        
        return in_zone, zone_quality
    ...
    def mean_reversion_strategy(self, df: pd.DataFrame, idx: int, vp: Dict) -> Optional[Dict]:
        """Mean reversion from VA boundaries back to POC - R9 TIGHTER ZONES"""
        if not vp or idx < 50:
            return None
        ...
        # R9: TIGHTER ZONES - Use 1.0x ATR instead of 2.0x
        zone_size = atr * self.config['entry_zone_atr']
        
        # Check if price is near VAH or VAL with zone quality
        near_vah, vah_quality = self.check_entry_zone(current['close'], vp['vah'], atr, 'standard')
        near_val, val_quality = self.check_entry_zone(current['close'], vp['val'], atr, 'standard')
        
        if not (near_vah or near_val):
            return None
        
        # R9: Only take high quality entries
        if near_vah and vah_quality < 0.5:
            near_vah = False
        if near_val and val_quality < 0.5:
            near_val = False
    ...
    def detect_breakout_signals(self, df, idx, vp):
        ...
        if prev_high <= vah * 1.01 and current_close > vah * 1.005:  # Clean break
            ...
            if volume_confirmed:  # Will be True if volume >= 1.5x
                ...
                if score >= 80:  # Only take high-quality breakouts
                    # R9: Use dynamic targets and wider stops
                    stop = self.calculate_stop_loss(df, vp, current_close, 'long', idx)
                    target = self.calculate_target(df, vp, current_close, 'long', 'breakout', idx)
                    
                    # Check R/R before adding signal
                    risk = current_close - stop
                    reward = target - current_close
                    rr_ratio = reward / risk if risk > 0 else 0
                    
                    if rr_ratio >= 1.2:  # Relaxed R/R requirement
                        if self.debug:
                            print(f"🎯 BREAKOUT SIGNAL: Long above VAH, score={score}, volume={volume_ratio:.2f}x, R/R={rr_ratio:.2f}")
                        ...
                        print(f"⛔ Breakout rejected - R/R {rr_ratio:.2f} < 1.2")
        ...
        # Check for breakout BELOW VAL
            ...
            if volume_confirmed:
                ...
                if score >= 80:
                    # R9: Use dynamic targets and wider stops
                    stop = self.calculate_stop_loss(df, vp, current_close, 'short', idx)
                    target = self.calculate_target(df, vp, current_close, 'short', 'breakout', idx)
                    ...
                    if rr_ratio >= 1.2:  # Relaxed R/R requirement
                        if self.debug:
                            print(f"🎯 BREAKOUT SIGNAL: Short below VAL, score={score}, volume={volume_ratio:.2f}x, R/R={rr_ratio:.2f}")
    ...
    def detect_mean_reversion_signals(self, df, idx, vp):
        """
        Enhanced mean reversion with better entry and exit logic
        R9: Zone quality filter and pre-filter R/R
        """
        signals = []
        ...
        # Long signals from VAL with zone quality
        in_zone, zone_quality = self.check_entry_zone(current_low, val, atr, 'standard')
        
        if in_zone and zone_quality > 0.4:  # Relaxed quality threshold
            score = self.calculate_entry_score(df, idx, vp, 'mean_reversion')
            
            if score >= 70:
                stop = self.calculate_stop_loss(df, vp, current_price, 'long', idx)
                target = self.calculate_target(df, vp, current_price, 'long', 'mean_reversion', idx)
                ...
                if rr_ratio >= 1.2:  # Relaxed R/R requirement
                    ...
                    print(f"⛔ Trade rejected - R/R {rr_ratio:.2f} < 1.2")
                
        # Short signals from VAH with zone quality
        in_zone, zone_quality = self.check_entry_zone(current_high, vah, atr, 'standard')
        ...
        if in_zone and zone_quality > 0.4:  # Relaxed quality threshold
            ...
            if score >= 70:
                ...
                target = self.calculate_target(df, vp, current_price, 'short', 'mean_reversion', idx)
        ...
        if 2.0 <= distance_pct <= 5.0:  # Research-based range
            ...
            if volume_confirmed:
                ...
                if score >= 80:
                    ...
                    # R9: Use dynamic targets and wider stops
                    stop = self.calculate_stop_loss(df, vp, current_price, signal_type, idx)
                    target = self.calculate_target(df, vp, current_price, signal_type, 'poc_magnet', idx)
                    ...
                    print(f"⛔ POC Magnet rejected - R/R {rr_ratio:.2f} < 1.2")
            ...
            if distance_pct <= 2.0:  # Close to virgin POC
                ...
                if volume_confirmed:
                    ...
                    if score >= 75:  # Lower threshold for virgin POC
                        ...
                        # R9: Use dynamic targets and wider stops
                        stop = self.calculate_stop_loss(df, vp, current_price, signal_type, idx)
                        target = self.calculate_target(df, vp, current_price, signal_type, 'virgin_poc', idx)
                        ...
                        if rr_ratio >= 1.2:  # Relaxed R/R requirement
                            ...
                            print(f"⛔ Virgin POC rejected - R/R {rr_ratio:.2f} < 1.2")
    ...
    def poc_magnet_strategy(self, df: pd.DataFrame, idx: int, vp: Dict) -> Optional[Dict]:
        ...
        # R9: Use dynamic targets and wider stops
        stop = self.calculate_stop_loss(df, vp, entry, direction, idx)
        target = self.calculate_target(df, vp, entry, direction, 'poc_magnet', idx)
        ...
        if rr_ratio < 1.2:  # R10: Relaxed 1.2:1 R/R
            return None
    ...
    def virgin_poc_strategy(self, df: pd.DataFrame, idx: int, vp: Dict) -> Optional[Dict]:
        ...
        for vpoc in virgin_pocs:
            if abs(current['close'] - vpoc) < atr * self.config['entry_zone_atr']:
                ...
                # R9: Use dynamic targets and wider stops
                stop = self.calculate_stop_loss(df, vp, entry, direction, idx)
                target = self.calculate_target(df, vp, entry, direction, 'virgin_poc', idx)
                ...
                if rr_ratio >= 1.5:  # R9: Minimum 1.5:1 R/R
                    return {
                        'strategy': 'virgin_poc',
                        'direction': direction,
                        'entry': entry,
    ...
    def composite_strategy(self, df: pd.DataFrame, idx: int, vp: Dict) -> Optional[Dict]:
        ...
        # R9: Use dynamic targets and wider stops
        stop = self.calculate_stop_loss(df, vp, entry, direction, idx)
        target = self.calculate_target(df, vp, entry, direction, 'composite', idx)

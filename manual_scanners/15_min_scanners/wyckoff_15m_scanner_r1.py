#!/usr/bin/env python3
"""
Wyckoff 15M Scanner R1 - Database Integration Wrapper
File: manual_scanners/15_min_scanners/wyckoff_15m_scanner_r1.py
...
try:
    from wyckoff_15m_scanner_r0 import WyckoffScanner15M
    BASE_SCANNER_AVAILABLE = True
    print("✅ Base Wyckoff 15M scanner (R0) imported successfully")
except ImportError as e:
    print(f"❌ Base scanner not available: {e}")
    BASE_SCANNER_AVAILABLE = False
    WyckoffScanner15M = object

try:
    from database_and_logging.wyckoff_logger import WyckoffLogger
    DB_LOGGING_ENABLED = True # Global variable for initial check
    print("✅ Database logging enabled for Wyckoff 15M scanner")
except ImportError as e:
    print(f"❌ Database logging not available: {e}")
    DB_LOGGING_ENABLED = False

class WyckoffScanner15MR1(WyckoffScanner15M):
    """Enhanced Wyckoff 15M Scanner with database integration"""
    def __init__(self):
        if BASE_SCANNER_AVAILABLE:
            super().__init__()
        self.timeframe = '15m'
        self.scanner_name = 'Wyckoff Scanner 15M R1'

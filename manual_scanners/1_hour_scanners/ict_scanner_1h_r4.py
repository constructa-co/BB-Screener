...
class ICTFVGScanner:
    ...
    def __init__(self, exchange_id='binance', test_mode=False):
        ...
        # R2 Proven Parameters (from successful backtest)
        self.config = {
            # FVG Detection (R2 optimized)
            'min_fvg_size': 0.004,          # 0.4% for 1H volatility (from 0.6%)
            'max_fvg_age': 72,              # 72 bars for 1H (from 30)
            'min_quality_score': 70,         # Minimum quality threshold
            'max_distance_to_fvg': 0.10,    # Max 10% distance from current price
            
            # Order Block Detection (R2 optimized)
            'min_order_block_size': 1.0,    # 1.0% for 1H (from 2.0)
            'min_volume_ratio': 1.5,        # 1.5x for 1H (from 2.5)
            'max_ob_age': 72,               # 72 bars for 1H (from 40)
            
            # Targets (R2 optimized)
            'targets': {
            ...
            # Risk Management (R2 optimized)
            'stop_loss_pct': 0.006,         # 0.6% stop loss (from 0.015)
...
"""
    ...
    def scan_all_symbols(self, top_n=500, min_quality=75, max_alerts=20):
        ...
        # Log trades to database using universal logger
        call_universal_logger(high_quality_setups)
...
def call_universal_logger(high_quality_setups):
    ...
    try:
        ...
        logger = UniversalScannerLogger('ict_1h_scanner', 'r4')
        
        if high_quality_setups:
            for setup in high_quality_setups[:20]:
                # Preserve original data BEFORE enrichment
                original_fvg = setup.get('fvg_zone', {})
                original_swings = setup.get('swing_levels', {})
                ...
                trade_data = {
                    ...
                    # Scanner signals - Use preserved original data
                    'scanner_signals': {
                        ...
                        'gap_high': float(original_fvg.get('high', 0)),
                        'gap_low': float(original_fvg.get('low', 0)),
                        'gap_midpoint': float(setup.get('entry_price', 0)),
                        'swing_high': float(original_swings.get('high', 0)),
                        'swing_low': float(original_swings.get('low', 0)),
                        'swing_range_pct': float(abs(original_swings.get('high', 0) - original_swings.get('low', 0)) / original_swings.get('low', 1) * 100),
                    ...
                    # Fibonacci targets - Use preserved original targets
                    'feature_vector': [
                        float(original_targets.get('T1', 0)),
                        float(original_targets.get('T2', 0)),
                        float(original_targets.get('T3', 0)),
                        float(original_targets.get('fib_236', 0)),
                        float(original_targets.get('fib_382', 0)),
                        float(original_targets.get('fib_500', 0)),
                        float(original_targets.get('fib_618', 0)),
                        float(original_targets.get('fib_786', 0))
                    ]
                }

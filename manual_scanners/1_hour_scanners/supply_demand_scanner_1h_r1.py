...
File: supply_demand_scanner_1h_r1.py
...
spec = importlib.util.spec_from_file_location("base_scanner", "/opt/bb-screener/manual_scanners/1_hour_scanners/supply_&_demand_scanner_1h_r0.py")
base_module = importlib.util.module_from_spec(spec)
spec.loader.exec_module(base_module)
BaseScanner = base_module.SupplyDemandScanner1H
...
class SupplyDemandScanner1HR1(BaseScanner):
    ...
    def _calculate_quality_score(self, zone: Dict, df: pd.DataFrame) -> int:
        ...
        if 1.0 <= zone_range <= 3.0:  # Optimal zone size
            score += 20
        elif 0.8 <= zone_range <= 4.0:
            score += 10
...
def main():
    """Main execution"""
    scanner = SupplyDemandScanner1HR1(timeframe='1h')

    ...
    def scan_all_symbols(self):
        ...
        for i, symbol in enumerate(symbols, 1):
            try:
                ...
                # Log patterns to Elliott Wave database
                for pattern in patterns:
                    if self.elliott_logger:
                        try:
                            signal_data = {
                                ...
                                'current_wave': pattern['current_wave'],
                                'wave_degree': 'minor',  # Default for 1H
                                'direction': pattern['direction'],
                                
                                # Trading levels
                                'entry_price': pattern['current_price'],
                                'stop_loss': pattern['wave_start'] * 0.95,  # Conservative stop
                                'targets': pattern['targets'],  # Logger handles both list and dict
                                'risk_reward': 2.5,  # Default R:R
                                
                                # Quality metrics
                                'pattern_quality': pattern['quality_score'],
                                'confidence_score': pattern['quality_score'] / 100.0,  # Convert to 0-1 scale
                                'invalidation_level': pattern['wave_start'] * 0.90,  # Below wave start
                                
                                # Wave analysis (full structure)
                                'waves': pattern['waves'],
                                'wave_analysis': {
                                    'wave_start': pattern['wave_start'],
                                    'wave_start_date': pattern['wave_start_date'],
                                    'duration': pattern['duration'],
                                ...
                                # Additional data
                                'volume': None,  # Not available in current scanner
                                'timeframe_str': '1-3 hours'
                            }
                            
                            # Generate scan_id for this run (once per scan)
                            if not hasattr(self, 'current_scan_id'):
                                self.current_scan_id = datetime.now().strftime("%Y%m%d%H%M%S")
                            
                            signal_id = self.elliott_logger.log_elliott_signal(
                                signal_data, 
                                self.current_scan_id
                            )
                            print(f"  📊 Logged to database with ID: {signal_id}")
        ...
        # Clean up logger at end
        if self.elliott_logger:
            try:
                self.elliott_logger.close()
                print("✅ Elliott Wave Logger connection closed")
            except:
                pass

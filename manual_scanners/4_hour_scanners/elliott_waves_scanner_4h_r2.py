#!/usr/bin/env python3
"""
Elliott Wave Scanner - 4H Chart (Exact Daily Logic with Minimal Changes)
Professional Elliott Wave pattern detection focused on current major cycles

ONLY CHANGES from daily scanner:
- Timeframe: '1d' → '4h'
- Lookback: 120 → 30 days
- Min wave size: 8% → 4%
- Volume threshold: $500K → $100K

ALL ELLIOTT WAVE LOGIC IDENTICAL to working daily scanner
"""
...
class ElliottWaveScanner:
    def __init__(self):
        ...
        # 4H parameters (minimal changes from daily)
        self.lookback_periods = 30   # 30 vs 120 for daily
        self.min_wave_size = 4       # 4% vs 8% for daily
        self.min_pivot_distance = 5  # Keep same as daily
        self.min_quality_score = 40  # Keep same as daily
        self.major_move_threshold = 15  # Keep same as daily
        
        # Initialize Elliott Wave logger (conditional)
        ...
        try:
            ...
            print("📊 Scanner will run without database logging")
        except Exception as e:
            print(f"⚠️ Failed to initialize Elliott Wave Logger: {e}")
            print("📊 Scanner will run without database logging")
        ...
        try:
            ...
            for symbol in usdt_pairs:
                try:
                    ...
                    # Minimum criteria for 4H analysis
                    min_volume_24h = 100000   # $100K+ vs $500K for daily
            ...
            # Take top liquid pairs for 4H analysis
            top_pairs = viable_pairs[:150]  # Same as daily
            
            print(f"📊 Filtered to {len(top_pairs)} high-liquidity pairs for 4H analysis")
    ...
    def get_daily_data(self, symbol):
        """Get 4H OHLCV data (ONLY CHANGE: timeframe and lookback)"""
        try:
    ...
    def scan_all_symbols(self):
        ...
        print("🌊 ELLIOTT WAVE SCANNER - 4H CHART (CURRENT STRUCTURE FOCUS)")
        print("=" * 85)
        print("📈 Scanning for current major Elliott Wave structures...")
        print("📊 Strategy: Current cycle analysis → Swing trend targets")
        print("💰 Filtering: $100K+ daily volume, top 150 most liquid pairs")
        print()
        ...
        # Log patterns to database if logger is available
        if self.elliott_logger and all_patterns:
            try:
                scan_id = f"elliott_4h_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
                print(f"\n💾 Logging {len(all_patterns)} patterns to database...")
                
                for pattern in all_patterns:
                    # Map pattern data to database fields
                    signal_data = {
                        ...
                        'current_wave': pattern['current_wave'],
                        'wave_degree': 'intermediate',  # 4H timeframe = intermediate degree
                        ...
                        # Wave analysis data
                        'waves': pattern['waves'],
                        'wave_relationships': pattern.get('wave_relationships', {}),
                        'internal_structure': pattern.get('internal_structure', {}),
                        'alternation': pattern.get('alternation', {}),
                        'wave_start': pattern.get('wave_start'),
                        'wave_start_date': pattern.get('wave_start_date'),
                        'current_price': pattern['current_price'],
                        'duration': pattern.get('duration'),
                        
                        # Fibonacci levels
                        'fibonacci_levels': pattern.get('fibonacci_levels', {}),
                        
                        # Pattern metrics
                        'strength_indicators': pattern.get('strength_indicators', {}),
                        'validation_checks': pattern.get('validation_checks', {}),
                        'wave_validation': pattern.get('wave_validation', {})
                    }
                    
                    self.elliott_logger.log_elliott_signal(signal_data, scan_id)
                
                print(f"✅ Successfully logged {len(all_patterns)} patterns to database")
                
            except Exception as e:
                print(f"⚠️ Database logging error: {e}")
                print("📊 Scanner completed successfully without database logging")
        
        # Close database connection if logger was used
        if self.elliott_logger:
            try:
                self.elliott_logger.close()

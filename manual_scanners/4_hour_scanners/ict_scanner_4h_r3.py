...
class ICTFVGScanner:
    ...
    def calculate_setup_details(self, symbol: str, fvg: Dict, current_price: float) -> Dict:
        ...
        if fvg['type'] == 'bullish':
            ...
            # Fibonacci extension targets from gap high
            t1 = fvg['gap_high'] + (gap_range * 0.618)   # 61.8% extension
            t2 = fvg['gap_high'] + (gap_range * 1.000)   # 100% extension
            t3 = fvg['gap_high'] + (gap_range * 1.618)   # 161.8% extension
            
        else:  # bearish
            # Stop loss just above the gap
            stop_loss = fvg['gap_high'] + (gap_range * 0.236)  # 23.6% above gap
            
            # Fibonacci extension targets from gap low
            t1 = fvg['gap_low'] - (gap_range * 0.618)    # 61.8% extension
            t2 = fvg['gap_low'] - (gap_range * 1.000)    # 100% extension
            t3 = fvg['gap_low'] - (gap_range * 1.618)    # 161.8% extension
...
🎯 FIBONACCI TARGETS:
• T1: ${setup['targets']['T1']:.4f} (+{t1_pct:.1f}%) [61.8% ext] - Exit 50%
• T2: ${setup['targets']['T2']:.4f} (+{t2_pct:.1f}%) [100% ext] - Exit 30%
• T3: ${setup['targets']['T3']:.4f} (+{t3_pct:.1f}%) [161.8% ext] - Exit 20%

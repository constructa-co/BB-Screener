...
File: supply_demand_scanner_4h_r1.py
...
spec = importlib.util.spec_from_file_location("base_scanner", "/opt/bb-screener/manual_scanners/4_hour_scanners/supply_&_demand_scanner_4h_r0.py")
base_module = importlib.util.module_from_spec(spec)
spec.loader.exec_module(base_module)
BaseScanner = base_module.SupplyDemandScanner4H
...
class SupplyDemandScanner4HR1(BaseScanner):
    ...
    def identify_zones(self, symbol: str, df: pd.DataFrame) -> List[Dict]:
        ...
        for zone in zones:
            ...
            enhanced_zone = {
                ...
                'volume_confirmation': zone.get('volume_ratio', 1.0) >= 1.3,
                ...
                'algorithm_parameters': {
                    ...
                    'min_move_pct': 3.0,  # Higher threshold for 4H
                    'volume_threshold': 1.5,  # Higher volume requirement for 4H
                    'zone_range_min': 1.0,  # Larger zones for 4H
                    'zone_range_max': 8.0
                }
            }
    ...
    def _calculate_quality_score(self, zone: Dict, df: pd.DataFrame) -> int:
        ...
        if 1.5 <= zone_range <= 4.0:  # Optimal zone size for 4H
            score += 20
        elif 1.0 <= zone_range <= 6.0:
            score += 10
    ...
    def _determine_formation_type(self, df: pd.DataFrame, zone: Dict) -> str:
        ...
        before_start = max(0, idx - 8)
        before_end = max(0, idx - 2)
        after_start = min(len(df), idx + 2)
        after_end = min(len(df), idx + 8)
...
def main():
    """Main execution"""
    scanner = SupplyDemandScanner4HR1(timeframe='4h')

...
class ICTFVGScanner:
    ...
    def find_swing_high(self, df: pd.DataFrame, lookback: int = 30) -> float:
        ...
        # Verify it's a proper swing (lower highs on both sides)
        for i in range(2, len(highs) - 2):
            if (highs[i] > highs[i-1] and highs[i] > highs[i-2] and 
                highs[i] > highs[i+1] and highs[i] > highs[i+2]):
                if highs[i] > swing_high * 0.95:  # Within 5% of max
                    swing_high = highs[i]
                    
        return swing_high
        
    def find_swing_low(self, df: pd.DataFrame, lookback: int = 30) -> float:
        ...
        # Verify it's a proper swing (higher lows on both sides)
        for i in range(2, len(lows) - 2):
            if (lows[i] < lows[i-1] and lows[i] < lows[i-2] and 
                lows[i] < lows[i+1] and lows[i] < lows[i+2]):
                if lows[i] < swing_low * 1.05:  # Within 5% of min
                    swing_low = lows[i]

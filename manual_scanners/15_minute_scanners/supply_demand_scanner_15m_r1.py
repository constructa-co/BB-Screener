...
File: supply_demand_scanner_15m_r1.py
...
spec = importlib.util.spec_from_file_location("base_scanner", "/opt/bb-screener/manual_scanners/15_min_scanners/supply_&_demand_scanner_15m_r0.py")
base_module = importlib.util.module_from_spec(spec)
spec.loader.exec_module(base_module)
BaseScanner = base_module.SupplyDemandScanner15M
...
class SupplyDemandScanner15MR1(BaseScanner):
    ...
    def identify_zones(self, symbol: str, df: pd.DataFrame) -> List[Dict]:
        ...
        for zone in zones:
            ...
            enhanced_zone = {
                ...
                'volume_confirmation': zone.get('volume_ratio', 1.0) >= 1.2,
                ...
                'algorithm_parameters': {
                    ...
                    'min_move_pct': 2.0,  # Lower threshold for 15M
                    'volume_threshold': 1.2,  # Lower volume requirement for 15M
                    'zone_range_min': 0.5,  # Smaller zones for 15M
                    'zone_range_max': 4.0
                }
            }
    ...
    def _calculate_quality_score(self, zone: Dict, df: pd.DataFrame) -> int:
        ...
        if 0.8 <= zone_range <= 2.5:  # Optimal zone size for 15M
            score += 20
        elif 0.5 <= zone_range <= 3.5:
            score += 10
...
def main():
    """Main execution"""
    scanner = SupplyDemandScanner15MR1(timeframe='15m')

...
class HeikinAshiRangeScanner:
    ...
    def analyze_ranging_quality(self, df, symbol=""):
        ...
        # PNUT-style pattern detection
        if range_pct < 2.0 or range_pct > 4.5:  # Tighter range like PNUT
            return None
        ...
        # Very horizontal like PNUT (PNUT had 0.1% trend)
        if trend_strength > 1.0:  # Was 2%, now 1%
            return None
        
        # Better support/resistance (like PNUT's clean levels)
        upper_zone = high_40 * 0.99   # Top 1%
        lower_zone = low_40 * 1.01    # Bottom 1%
        ...
        # Check for PNUT-style oscillation quality
        # Look for time spent bouncing between levels (not stuck in middle)
        extreme_zone_time = len(recent_df[
            (recent_df['close'] <= low_40 * 1.01) | 
            (recent_df['close'] >= high_40 * 0.99)
        ])
        
        # PNUT spends time at levels, not just middle
        if extreme_zone_time < 8:  # Need at least 8/40 touches at extremes
            return None
        
        # Volume consistency (PNUT has stable volume)
        if recent_df['volume'].std() / recent_df['volume'].mean() > 1.2:
            return None
        ...
        return {
            ...
            'extreme_zone_time': extreme_zone_time,
        }
    ...
    def scan_for_opportunities(self):
        ...
        print("=" * 60)
        print("🔍 Scanning for PNUT-STYLE oscillating ranges...")
        print("🎯 Target: Clean 2-3.5% ranges with multiple level bounces")
        print()
        ...
        for i, symbol in enumerate(coins):
            try:
                ...
                # PNUT-style scoring (rewards clean oscillation)
                score = 30  # Lower base score
                
                # Perfect PNUT-style range (2.0-3.5%)
                if 2.0 <= range_analysis['range_pct'] <= 3.5:
                    score += 40  # High reward for PNUT-like range
                elif range_analysis['range_pct'] <= 4.5:
                    score += 25  # Good range
                
                # Super horizontal (PNUT had 0.1% trend)
                if range_analysis['trend_strength'] < 0.3:
                    score += 35  # Perfect like PNUT
                elif range_analysis['trend_strength'] < 0.7:
                    score += 25  # Very good
                elif range_analysis['trend_strength'] < 1.0:
                    score += 15  # Acceptable
                
                # Level interaction (key for PNUT-style)
                total_touches = range_analysis['upper_touches'] + range_analysis['lower_touches']
                extreme_time = range_analysis['extreme_zone_time']
                
                if total_touches >= 8 and extreme_time >= 12:
                    score += 25  # Excellent level interaction
                elif total_touches >= 6 and extreme_time >= 10:
                    score += 20  # Good interaction
                elif total_touches >= 4 and extreme_time >= 8:
                    score += 10  # Acceptable
                
                # HA color changes (PNUT had good scalping activity)
                if 2 <= ha_analysis['color_changes'] <= 5:
                    score += 15  # Perfect for scalping
                elif ha_analysis['color_changes'] >= 1:
                    score += 5   # Some activity
                
                # Higher threshold for PNUT-quality only
                if score < 90:  # Was 75, now 90 for premium quality
                    continue
                ...
                opportunities.append({
                    ...
                    'current_bullish': ha_analysis['current_bullish'],
                                            'extreme_zone_time': range_analysis['extreme_zone_time'],
                })
    ...
    def display_results(self, opportunities):
        ...
        for i, opp in enumerate(opportunities[:15], 1):
            ...
            print(f"     Setup: {opp['setup_type']} ({opp['confidence']}%) | "
                  f"Touches: {opp.get('upper_touches', '?')}U/{opp.get('lower_touches', '?')}L | "
                  f"Extreme: {opp.get('extreme_zone_time', '?')}/40")
            print()
